...
export const award = async (userId: string, type: 'subscribe' | 'order') => {
  ...
  if (foundUser) {
    ...
    let text = '';
    if (type === 'subscribe') {
      text += `🎉 有用户通过你的推荐码关注了公众号啦，${bonus.bonus}N币已到账 ${getTextReplyUrl('查询')}

该用户下单AI群年卡后，你可以可获得500N币奖励  ${getTextReplyUrl('奖励规则')}
...
`;
    ...
    if (type === 'order') {
      text += `🎉 有用户通过你的推荐码下单了AI群年卡，${bonus.bonus}N币已到账 ${getTextReplyUrl('查询')}

推荐给更多用户，继续获得奖励 ${getTextReplyUrl('奖励规则')}
